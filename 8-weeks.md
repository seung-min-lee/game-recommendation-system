...
### 5. LightGCN 시각화 개선 과정

---

#### 5-1. LightGCN 시각화 방식 사전 조사

- **조사 배경**
  - 기존 그래프 시각화가 단순 노드·엣지 나열 수준 → 추천 결과와 연결 관계가 직관적으로 드러나지 않음
  - 전문가·논문에서 LightGCN을 어떻게 시각화하는지 벤치마킹 후 개선 방향 수립

- **조사 내용 — 전문가 시각화 접근 방식**
  - **이분 그래프(Bipartite Graph) 시각화**
    - LightGCN 원 논문(He et al., SIGIR 2020) 및 후속 연구에서 유저 노드·아이템 노드를 좌우 분리 배치하는 2컬럼 레이아웃이 표준
    - 엣지는 유저-아이템 상호작용(구매·플레이)을 표현하며, 가중치(구매 횟수·플레이타임)를 두께로 표현하는 방식 채택
  - **임베딩 공간 시각화 (t-SNE / UMAP)**
    - 학습된 임베딩 벡터를 2차원으로 투영하여 아이템 클러스터 분포 확인
    - RecSys·SIGIR 논문에서 t-SNE로 아이템 임베딩 군집화를 시각화하여 모델 품질 정성 평가하는 방식이 널리 사용됨
    - UMAP은 t-SNE 대비 전역 구조 보존 강점으로 최근 추천 시스템 논문에서 채택 증가
  - **노드 크기·색상 인코딩**
    - 유저 활동량(플레이타임), 아이템 인기도(플레이 유저 수)를 노드 크기에 인코딩
    - 장르·카테고리별 색상 구분으로 군집 패턴 시각화

- **조사 결과 도출 개선 방향**
  - ① 유저·보유 게임·추천 게임을 3컬럼으로 분리 배치 (이분 그래프 표준 레이아웃 적용)
  - ② 플레이타임 가중 엣지 두께 적용 (전문가 방식 채택)
  - ③ t-SNE/UMAP 임베딩 분포 시각화 탭 추가 (논문 정성 평가 방식 도입)

---

#### 5-2. t-SNE 시각화 탭 추가 및 제거

- **추가 내용**
  - 추천 페이지에 **🧠 취향 지도** 독립 탭(tab6) 신설
  - 게임 임베딩 벡터를 PCA(15차원) → t-SNE(2차원) 2단계 축소 후 Plotly scatter로 렌더링
  - 추천 게임(빨간 별) / 보유 게임(파란 사각형) / 일반 게임(장르색 원) 노드 유형 구분
  - 유사 유저 유사도 순위 테이블 탭 하단 병기

- **제거 결정 및 근거**
  - 독립 탭으로 분리 시 추천 탭과 맥락 단절 → 그래프 맥락 없이 임베딩 분포만 단독 노출되어 의미 전달 부족
  - 탭 수 과다(6개)로 UI 복잡도 증가
  - **결정:** 독립 탭 제거 후 LightGCN expander 내부 필터로 통합하는 방향으로 전환

---

#### 5-3. LightGCN 그래프 전면 재설계

- **문제점 (기존 구현)**
  - 노드·엣지 각각 개별 Plotly trace 생성 → 유저 10명 × 게임 수십 개 = 수백 개 trace 생성
  - 렌더링 지연 심각, 브라우저 메모리 점유 과다
  - 모든 엣지 동일 두께·색상 → 추천 게임과 보유 게임 연결선 시각적 구분 불가
  - 배경 밝아 glow 효과 없음 → 노드·엣지 가독성 낮음

- **개선 ① — Batch Trace 렌더링 최적화**
  - 엣지를 연결 유형별 3개 batch trace로 통합 (수백 개 → ~10개)
    - 타 유저 연결 엣지: `rgba(255,220,50,0.35)` 노란색 반투명
    - 내 보유 게임 연결 엣지: `rgba(120,180,255,0.70)` 파란색
    - 내 추천 게임 연결 엣지: `rgba(255,80,80,0.90)` 빨간색 강조
  - 노드도 유형별 4종 batch scatter trace로 통합 (유저 / 보유 게임 / 추천 게임 / 기타 게임)
  - 결과: 렌더링 속도 대폭 향상, 브라우저 부하 감소

- **개선 ② — 플레이타임 가중 엣지 두께**
  - 사전 조사에서 벤치마킹한 전문가 방식 적용
  - 플레이타임에 `log1p` 함수 적용 후 최대값 기준 정규화
  - 수식: `edge_width = 0.5 + 3.5 × (log1p(playtime) / log1p(max_playtime))`
  - `log1p` 채택 이유: 플레이타임 분포 극단적 편향(1시간 ~ 85,000시간) → 선형 스케일 시 대부분 엣지가 얇게 붙어 구분 불가, 로그 스케일로 시각적 분포 균등화

- **개선 ③ — Glow 이중 레이어 효과**
  - 추천 게임 연결 엣지에 두 선 중첩
    - 배경 레이어: width=8, opacity=0.3 반투명 넓은 선 (glow 번짐 효과)
    - 전경 레이어: width=2, opacity=0.9 불투명 얇은 선 (선명한 핵심 선)
  - 배경색 `#0e0e0e` (거의 검정) 적용 → glow 발광 대비 극대화
  - 추천 게임 노드: 빨간 별(⭐) 라벨 + 동일 glow 이중 레이어 원형 마커 적용

- **개선 ④ — 3컬럼 레이아웃 (이분 그래프 표준 적용)**
  - `x=0`: 유저 노드 (로그인 유저 + 그래프 참여 유저, 세로 균등 배치)
  - `x=1`: 보유 게임 및 기타 게임 노드
  - `x=2`: 추천 게임 노드 (우측 분리, 세로 균등 배치)
  - 컬럼 구분 수직선(`shapes`) 추가 → 이분 그래프 구조 경계 명확화
  - 그래프 높이 700px, `margin` 최소화로 노드 가시 영역 최대 확보

---

#### 5-4. LightGCN 그래프 위치 이동 및 Collapsible Expander 적용

- **변경 전 구조**
  - 추천 탭 진입 시 LightGCN 그래프가 최상단 노출 → 캐러셀(추천 게임 목록) 보기 전 스크롤 필요
  - 그래프 항상 펼쳐져 있어 700px 높이로 페이지 스크롤 UX 저하

- **변경 내용**
  - 그래프를 캐러셀 + 리뷰 패널 하단으로 이동 → 추천 게임 목록 먼저 확인 후 그래프 탐색 가능
  - `st.expander("🕸️ LightGCN 그래프 & 임베딩 시각화")` 로 감싸 기본 접힌 상태 유지
  - 필요할 때만 펼쳐 확인 → 불필요한 렌더링 방지 및 페이지 길이 단축

---

#### 5-5. LightGCN Expander 내 UMAP/t-SNE 라디오 필터 통합

- **통합 구조**
  - 독립 탭으로 제거했던 t-SNE 시각화를 LightGCN expander 내부로 재배치
  - expander 상단 라디오 버튼으로 시각화 모드 전환
    - `🕸️ LightGCN`: 유저-게임 이분 그래프 (기본값)
    - `🧭 t-SNE`: PCA → t-SNE 임베딩 분포
    - `🗺️ UMAP`: PCA → UMAP 임베딩 분포

- **`_render_embedding_viz(algo, rec_ids)` 통합 함수 구현**
  - `algo` 파라미터로 `"tsne"` / `"umap"` 분기 처리
  - 공통 전처리: 게임 벡터 수집 → PCA 15차원 선압축
  - t-SNE 분기: `sklearn.manifold.TSNE(perplexity=30, n_iter=300)` 적용
  - UMAP 분기: `umap.UMAP(n_neighbors=15, min_dist=0.1)` 적용
  - 결과 2차원 좌표로 Plotly scatter 렌더링 (노드 유형별 색상·마커 구분)

- **세션 캐시 분리 운영**
  - `st.session_state["_embed_tsne_cache"]`: t-SNE 계산 결과 독립 저장
  - `st.session_state["_embed_umap_cache"]`: UMAP 계산 결과 독립 저장
  - t-SNE ↔ UMAP 전환 시 각 캐시 재사용 → 전환 속도 개선, 불필요한 재계산 방지

- **라디오 레이블 수정**
  - 기존: `이분 그래프` → 변경: `LightGCN`
  - 변경 이유: 기술 용어(이분 그래프)보다 모델명(LightGCN)이 사용자에게 직관적
